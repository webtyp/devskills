---
name: components
description: WebTyp component creation standards for webtyp/components. Use when creating or reviewing UI components: file structure, embedding rules, CSS tokens, SSR split, icon registration, and PLAN.md workflow.
---
...
// OnMount is called by webtyp/dom after HTML is injected into the DOM.
...
`RenderCSS()` ships component-scoped CSS only — it goes to `sitec`'s `middle` slot. Components MUST NOT declare a `RootCSS()` function: theme `:root` tokens are global state owned by the app or `webtyp/dom`'s default theme. A third-party `RootCSS()` is silently ignored by `sitec` with a warning (single-override rule).
...
- A component MUST NOT define `:root { … }` — that block is owned by the app's `RootCSS()` (or `webtyp/dom`'s default fallback). Components only consume tokens, never declare them.
- No form-related CSS — use `webtyp/form` for that.

Theme tokens are defined in `webtyp/dom/theme.css` and injected into `<head>` by `sitec` via `dom.RootCSS()`. Available tokens:
```
...
Use only webtyp ecosystem modules — never `errors`, `strconv`, `strings`, `time`, `net/http`, `encoding/json`.
